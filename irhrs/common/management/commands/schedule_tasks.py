...
class Command(BaseCommand):
    SCHEDULES_DATA_PER_MODULE = {
        ...
        'attendance': [
            ...
            OrderedDict(
                [
                    ('name', 'Correct Leave TimeSheets'),
                    ('func',
                     'irhrs.attendance.tasks.timesheets.correct_leave_timesheets'),
                    ('schedule_type', 'D'),
                ]),
            OrderedDict(
                [
            ...
            OrderedDict(
                [
                    ...
                    ('func', 'irhrs.attendance.tasks.send_notifications.send_weekly_attendance_report_email'),
                    ('schedule_type', 'W'),
    ...
    def handle(self, *args, **options):
        ...
        for module in installs:
            proceed = no_input or (input(
                CGREEN
                + "Do you want to seed background tasks for "
                + f"{module} (Recommended) (Y/N)"
                + "\u001b[0m"
                + '\t'
            ).upper() == 'Y')
            if not proceed:
